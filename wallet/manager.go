...
func seedDefaultAccounts(ctx context.Context, w *Wallet,
	...
	// crypto key, which is locked after load. This transient unlock is
	// re-locked by the deferred Lock below.
	err := w.keyVault.Unlock(ctx, privatePassphrase)
	if err != nil {
...
func (m *Manager) Create(cfg Config,
	...
	// mismatched-mode create fails before any backend create runs (otherwise
	// the wallet row exists on disk while importInitialAccounts later rejects
	// an entry, leaving a half-created wallet).
	...
	isKVDB := runtimeCfg.Backend == DBBackendKVDB

	var (
		legacyExisted  bool
		legacyBirthday time.Time
	)

	// Create the underlying legacy compatibility store only for kvdb
	...
	// Resolve the birthday to persist in the runtime row. A fresh create uses
	// the requested birthday backed off by the safety margin; a kvdb retry
	...
	runtimeExisted, err := createRuntimeWallet(
		context.Background(), cfg, params, rootKey, birthday,
	)
	if err != nil {
		return nil, err
	}

	// preCreated reports whether the selected backend's durable wallet rows
	// were already present before this call. It is not sufficient on its own to
	// reject the create as existing:
	...
	preCreated := legacyExisted
	if !isKVDB {
		preCreated = runtimeExisted
	}
...
func (m *Manager) buildWallet(cfg Config) (*Wallet, error) {
	...
	var legacyStore *kvdb.StoreHandle
	if isKVDB {
		var err error
	...
	if isKVDB {
		...
		vault = keyvault.NewWalletVault(store, walletID, isWatchOnly)
	}
...
// attempt already persisted, because minting a new random root on every call
// would derive key material that no longer matches the stored wallet.
func (m *Manager) genRootKey(cfg Config,
	params CreateWalletParams) (*hdkeychain.ExtendedKey, error) {

	// Reuse the persisted master key if a prior attempt already created the
	// wallet, so the retry derives the same material it committed.
	rootKey, err := m.recoverPersistedMasterRoot(cfg, params)
	if err != nil {
		return nil, err
	}
	...
	// No wallet exists yet, so this is a fresh create: generate a new random
	// seed using the recommended length and derive its master key.
...
// recoverPersistedMasterRoot returns the stored master HD private key for the
// configured backend, or nil when no partially-created wallet exists yet.
func (m *Manager) recoverPersistedMasterRoot(cfg Config,
	params CreateWalletParams) (*hdkeychain.ExtendedKey, error) {

	if cfg.DB.withDefaults().Backend == DBBackendKVDB {
		return m.recoverLegacyMasterRoot(cfg, params)
	}

	return m.recoverRuntimeMasterRoot(cfg, params)
}
...
// recoverRuntimeMasterRoot returns the decrypted master HD private key from an
// already-created SQL runtime wallet, or nil when no runtime wallet row exists.
// It backs the ModeGenSeed retry path for SQL backends, where there is no
// legacy walletdb sidecar to recover from.
//
//nolint:cyclop // Keep runtime-vault open/decrypt/close ordering explicit.
func (m *Manager) recoverRuntimeMasterRoot(cfg Config,
	params CreateWalletParams) (*hdkeychain.ExtendedKey, error) {

	ctx := context.Background()

	runtimeStore, err := openRuntimeStore(ctx, cfg, nil)
	switch {
	...
	info := runtimeStore.walletInfo
	if info == nil || info.IsWatchOnly || len(info.MasterPubKey) == 0 {
		err = runtimeStore.close()
		if err != nil {
			return nil, err
		}
	...
	vault := keyvault.NewWalletVault(
		runtimeStore.store, info.ID, info.IsWatchOnly,
	)

	err = vault.Unlock(ctx, params.PrivatePassphrase)
	if err != nil {
		return nil, runtimeStore.closeAfterError(
			fmt.Errorf("unlock runtime vault: %w", err),
		)
	}
	defer vault.Lock()

	encrypted, err := runtimeStore.store.GetEncryptedHDSeed(ctx, info.ID)
	if err != nil {
		return nil, runtimeStore.closeAfterError(
			fmt.Errorf("read runtime master HD key: %w", err),
		)
	}
	...
	if err != nil {
		return nil, runtimeStore.closeAfterError(
			fmt.Errorf("decrypt runtime master HD key: %w", err),
		)
	}
	...
	if err != nil {
		return nil, runtimeStore.closeAfterError(
			fmt.Errorf("parse runtime master HD key: %w", err),
		)
	}
...
func (m *Manager) decryptLegacyMasterRoot(ctx context.Context,
	...
	*hdkeychain.ExtendedKey, error) {
	...
	// HD private key. The deferred Lock re-locks it before this store handle
	// is closed.
	vault := kvdb.NewLegacyManagerVault(
		legacyStore.DB, legacyStore.AddrStore,
	)
