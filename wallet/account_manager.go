...
func (w *Wallet) NewAccount(_ context.Context, scope waddrmgr.KeyScope,
	...
	if err != nil {
		return nil, fmt.Errorf(
			"failed to fetch scoped key manager for scope %s: %w",
			scope, err,
		)
	}
	...
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			return fmt.Errorf(
				"failed to get account properties: %w", err,
			)
		}
...
func (w *Wallet) ListAccounts(_ context.Context) (*AccountsResult, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		// Now, iterate through each key scope to assemble the final
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		for _, scopeMgr := range scopes {
			...
			if err != nil {
				...
				return fmt.Errorf(
					"failed to lookup account: %w", err,
				)
			}
			...
			if err != nil {
				return fmt.Errorf(
					"failed to get account properties: %w",
					err,
				)
			}
...
func (w *Wallet) Balance(_ context.Context, conf int32,
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		if err != nil {
			return fmt.Errorf(
				"failed to fetch scoped key manager for "+
					"scope %s: %w", scope, err,
			)
		}
		...
		if err != nil {
			return fmt.Errorf(
				"failed to get unspent outputs: %w", err,
			)
		}
