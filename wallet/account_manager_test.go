...
func TestGetAccountErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "cancellation with missing account",
			storeErr: errors.Join(
				context.Canceled, db.ErrAccountNotFound,
			),
...
func TestNewAccount(t *testing.T) {
	...
	// Assert: The public result reports the allocated account number.
	require.NoError(t, err)
...
func TestNewAccountWatchOnly(t *testing.T) {
	...
	_, err := w.NewAccount(t.Context(), scope, occupied)
...
func TestNewAccountLocked(t *testing.T) {
	...
	_, lockedErr := w.NewAccount(t.Context(), scope, testAccountName)
	_, invalidErr := w.NewAccount(t.Context(), scope, "")
...
func TestRenameAccountConflicts(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// Assert: Self-renames and occupied targets both refuse mutation.
			require.ErrorIs(t, err, ErrAccountAlreadyExists)
		})
	}
}
