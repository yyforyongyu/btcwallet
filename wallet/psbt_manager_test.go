...
func TestValidateDerivation(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "valid BIP32 input",
			// Arrange: Input with a single BIP32 derivation.
			pInput: &psbt.PInput{
				Bip32Derivation: []*psbt.Bip32Derivation{{}},
			},
			isTaproot: false,
		...
		{
			name: "valid Taproot input",
			// Arrange: Input with a single Taproot derivation.
			pInput: &psbt.PInput{
				TaprootBip32Derivation: taprootDerivation,
			},
			isTaproot: true,
		...
		{
			name: "multiple BIP32 derivation paths",
			// Arrange: Input with multiple BIP32 derivations.
			pInput: &psbt.PInput{
				Bip32Derivation: multiBip32Derivation,
			},
			isTaproot: false,
			// Assert: Expect an error for multiple BIP32
			// derivations.
			err: ErrUnsupportedMultipleBip32Derivation,
		},
		{
			name: "multiple Taproot derivation paths",
			// Arrange: Input with multiple Taproot derivations.
			pInput: &psbt.PInput{
				TaprootBip32Derivation: multiTapDerivation,
			},
			isTaproot: false,
			// Assert: Expect an error for multiple Taproot
			// derivations.
			err: ErrUnsupportedMultipleTaprootDerivation,
		},
		{
			name: "ambiguous derivation (both present)",
			// Arrange: Input with both BIP32 and Taproot
			// derivations.
			pInput: &psbt.PInput{
			...
			isTaproot: false,
			// Assert: Expect an error for ambiguous derivation.
			err: ErrAmbiguousDerivation,
		},
		{
			name: "no derivation info (valid state for " +
				"validation check itself, returns false/nil)",
			// Arrange: Input with no derivation information.
			pInput:    &psbt.PInput{},
			isTaproot: false,
			err:       nil,
		},
...
func TestFetchPsbtUtxo(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// and input index.
			got := fetchPsbtUtxo(tc.packet, tc.inputIdx)

			// Assert: Verify that the function returns the correct
			// UTXO, prioritizing WitnessUtxo if present, otherwise
			// falling back to NonWitnessUtxo.
