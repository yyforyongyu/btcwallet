...
// expectDerivedSignerPrivKeyAfterCacheMiss wires the signer-private key lookup
// path for a derived managed pubkey address after the account-key cache misses.
func expectDerivedSignerPrivKeyAfterCacheMiss(t *testing.T,
	mocks *mockWalletDeps, scope waddrmgr.KeyScope,
	path waddrmgr.DerivationPath, privKey *btcec.PrivateKey) {
...
func TestGetPrivKeyForAddressFallsBackAfterCacheMiss(t *testing.T) {
	...
	expectDerivedSignerPrivKeyAfterCacheMiss(
		t, mocks, waddrmgr.KeyScopeBIP0084, path, privKey,
	)
