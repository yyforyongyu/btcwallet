...
func (w *Wallet) finalizeInput(ctx context.Context, packet *psbt.Packet,
	...
	if utxo == nil {
		...
		// a panic in case of unexpected data inconsistency, rather than
		// returning a fatal error that would abort the entire process.
		log.Errorf("Input %d has no UTXO info", idx)
		return nil
	}
...
func addScriptToPInput(pInput *psbt.PInput,
	...
	if len(unlockingScript.Witness) > 0 {
		...
		err := psbt.WriteTxWitness(
			&witnessBuf, unlockingScript.Witness,
		)
		if err != nil {
