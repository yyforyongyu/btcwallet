...
// validateLegacyKVDB checks the kvdb database path when the resolved backend
// uses kvdb. SQL backends may run without any walletdb sidecar, unless they
// rely on KVDB.DBPath only to derive a default SQLite path.
func (c DBConfig) validateLegacyKVDB() error {
	if c.withDefaults().Backend != DBBackendKVDB {
		return nil
	}
