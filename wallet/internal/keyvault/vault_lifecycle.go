...
// passphrase carried by params. It preserves the vault's original
// locked/unlocked state: the runtime keys held while unlocked are never
// touched, and a locked vault leaves no decrypted state behind because the
// rotation reads and re-encrypts only the persisted ciphertext.
//
// The store-backed vault has no separate public passphrase, so requesting a
// public change returns errPublicPassphraseUnsupported. A request that changes
// neither passphrase is a no-op.
func (v *WalletVault) ChangePassphrase(ctx context.Context,
	params ChangePassphraseParams) error {

	// A store-backed vault protects its secrets with a single (private)
	// passphrase. There is no public passphrase to rotate, so reject the
	...
	// Lock/Unlock/Encrypt/Decrypt. The rotation itself never reads or writes
	// v.unlockedState, so the vault's locked/unlocked state is preserved
	...
	updateParams, err := v.makeRotatedWalletSecrets(
		secrets, params.PrivateOld, params.PrivateNew,
	)
	if err != nil {
		return fmt.Errorf("wallet %d vault ChangePassphrase: "+
...
func (v *WalletVault) makeRotatedWalletSecrets(secrets *db.WalletSecrets,
	oldPassphrase, newPassphrase []byte) (db.UpdateWalletSecretsParams, error) {
	...
	// Derive the old master private key from the stored parameters. DeriveKey
	// validates oldPassphrase against the stored digest, so a wrong passphrase
	// is rejected here before any secret material is touched.
	...
	// Re-encrypt the script crypto key under the new master key. Every wallet,
	// watch-only included, holds a script crypto key.
	...
	// A watch-only wallet has no private crypto key or HD root material, so the
	// script key is all it needs.
	if v.watchOnly {
		return db.UpdateWalletSecretsParams{
			WalletID:                 v.walletID,
	...
	// Re-encrypt the private crypto key under the new master key. Decrypting it
	// with the old master key also confirms oldPassphrase can reproduce the
	// spendable key material, beyond the digest check performed above.
	...
	return db.UpdateWalletSecretsParams{
		...
		// The master HD private key is encrypted under the crypto private key,
		// not the passphrase, so its ciphertext is carried forward unchanged.
...
func reEncryptCryptoKey(oldMasterKey, newMasterKey *snacl.SecretKey,
	...
	if len(decrypted) != snacl.KeySize {
		return nil, fmt.Errorf("decrypt crypto key expected %d bytes, "+
			"got %d: %w", snacl.KeySize, len(decrypted), errUnexpectedState)
	}
