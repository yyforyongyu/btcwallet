...
func CreateWalletSecrets(privatePassphrase []byte,
	...
	// key; Marshal persists the parameters (salt + N/R/P), not the key itself,
	// so Unlock can re-derive it from the same passphrase.
	masterKey, err := snacl.NewSecretKey(
		&privatePassphrase, snacl.DefaultN, snacl.DefaultR, snacl.DefaultP,
	)
	if err != nil {
	...
	// Every wallet, watch-only included, protects scripts with a script crypto
	// key encrypted under the master key.
	...
	// A watch-only wallet has no private signing material, so the script key is
	// all it needs.
	if watchOnly {
		return secrets, nil
	}
