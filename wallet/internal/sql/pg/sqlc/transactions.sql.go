...
WHERE
    t.wallet_id = $1
    AND u.spent_by_tx_id = any($2::BIGINT [])
...
func (q *Queries) ListOwnedInputsBySpendingTxIDs(ctx context.Context, arg ListOwnedInputsBySpendingTxIDsParams) ([]ListOwnedInputsBySpendingTxIDsRow, error) {
	rows, err := q.query(ctx, q.listOwnedInputsBySpendingTxIDsStmt, ListOwnedInputsBySpendingTxIDs, arg.WalletID, pq.Array(arg.TxIds))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
