...
func (w *PostgresWalletDB) GetAccount(ctx context.Context,
	...
	return getAccountByQuery(ctx, query,
		getQueries.byNumber, getQueries.byName)
}
...
func (w *PostgresWalletDB) ListAccounts(ctx context.Context,
	...
	return listAccountsByQuery(ctx, query,
		listQueries.byScope, listQueries.byName,
		listQueries.all)
}
...
func (w *PostgresWalletDB) RenameAccount(ctx context.Context,
	...
	return renameAccountByQuery(ctx, params,
		renameQueries.byNumber, renameQueries.byName)
}
...
func (w *PostgresWalletDB) CreateDerivedAccount(ctx context.Context,
	...
	err := w.ExecuteTx(ctx, func(qtx *sqlcpg.Queries) error {
		scopeID, err := pgEnsureKeyScope(ctx, qtx, params.WalletID,
			params.Scope)
		if err != nil {
			return err
		}
		...
		row, err := qtx.CreateDerivedAccount(ctx,
			sqlcpg.CreateDerivedAccountParams{
				ID:          scopeID,
		...
		if !row.AccountNumber.Valid {
			// This should never happen unless the query is
		...
		if err != nil {
			return fmt.Errorf("%w: %w", ErrMaxAccountNumberReached,
				err)
		}

		info = buildAccountInfo(accNumber, params.Name, DerivedAccount,
			false, params.Scope)
...
func pgGetAccountProps(ctx context.Context, qtx *sqlcpg.Queries,
	...
	return accountPropsRowToProps(
		row.AccountNumber, row.AccountName, int64(row.OriginID),
		row.EncryptedPublicKey, row.MasterFingerprint, row.IsWatchOnly,
		row.Purpose, row.CoinType,
		row.InternalTypeID, row.ExternalTypeID,
		idToAddressType[int16],
	)
}
...
func pgEnsureKeyScope(ctx context.Context, qtx *sqlcpg.Queries,
	walletID uint32, scope KeyScope) (int64, error) {

	return ensureKeyScope(ctx,
		qtx.GetKeyScopeByWalletAndScope,
...
// identical fields.
func pgAccountRowToInfo[T pgAccountInfoRow](row T) (*AccountInfo,
	error) {
	...
	return accountRowToInfo(base.AccountNumber, base.AccountName,
		int64(base.OriginID), base.IsWatchOnly, base.Purpose,
		base.CoinType)
}
...
func (p pgAccountListQueries) byScope(ctx context.Context,
	...
	return listAccounts(ctx, p.q.ListAccountsByWalletScope,
...
func (p pgAccountListQueries) byName(ctx context.Context,
	...
	return listAccounts(ctx, p.q.ListAccountsByWalletAndName,
...
func (p pgAccountListQueries) all(ctx context.Context,
	...
	return listAccounts(ctx, p.q.ListAccountsByWallet,
		int64(query.WalletID), pgAccountRowToInfo)
}
...
func (p pgAccountGetQueries) byNumber(ctx context.Context,
	...
	return getAccount(ctx, p.q.GetAccountByWalletScopeAndNumber,
...
func (p pgAccountGetQueries) byName(ctx context.Context,
	...
	return getAccount(ctx, p.q.GetAccountByWalletScopeAndName,
...
func (p pgAccountRenameQueries) byNumber(ctx context.Context,
	...
	return renameAccount(ctx,
		p.q.UpdateAccountNameByWalletScopeAndNumber,
...
func (p pgAccountRenameQueries) byName(ctx context.Context,
	...
	return renameAccount(ctx,
		p.q.UpdateAccountNameByWalletScopeAndName,
