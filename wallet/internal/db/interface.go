...
// address index. The callback receives the key scope and BIP44 account number,
// not the database account row ID.
type AddressDerivationFunc func(ctx context.Context, scope KeyScope,
	accountNumber uint32, branch uint32,
	index uint32) (*DerivedAddressData, error)
