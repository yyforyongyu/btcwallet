...
var (
	...
	// ErrUnknownKeyScope is returned when a key scope is not found
	// in ScopeAddrMap.
	ErrUnknownKeyScope = errors.New(
		"unknown scope in ScopeAddrMap",
	)

	// ErrInvalidAccountQuery is returned when both or neither account
	// filters are provided in GetAccount or RenameAccount.
	ErrInvalidAccountQuery = errors.New(
		"exactly one of Name or AccountNumber must be provided",
	)

	// ErrMissingAccountPublicKey is returned when an imported account
	// is missing the encrypted public key.
	...
	// ErrMaxAccountNumberReached indicates that no more accounts can
	// be created within a key scope because the account number counter
	// has reached its maximum representable value.
...
type AccountStore interface {
	...
	// with NULL encrypted keys using the address schema from
	// ScopeAddrMap.
