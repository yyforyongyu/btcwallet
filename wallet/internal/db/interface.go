...
type WalletStore interface {
	...
	// error if the wallet is not found or an unexpected issue occurs.
	...
	// birthday, birthday block, or sync state. The specific fields to update
	// are provided in the UpdateWalletParams struct. It returns an error if
	// the update fails.
	UpdateWallet(ctx context.Context, params UpdateWalletParams) error

	// GetEncryptedHDSeed retrieves the encrypted Hierarchical Deterministic
	// (HD) seed of the wallet. This seed is sensitive information and is
	// returned in its encrypted form. It returns the encrypted seed as a
	// byte slice or an error if the retrieval fails.
...
type AccountStore interface {
	...
	// returns the properties of the newly created account or an error if the
	// creation fails.
	...
	// import fails.
	ImportAccount(ctx context.Context, params ImportAccountParams) (*ImportAccountResult, error)

	// GetAccount retrieves information about a specific account, identified
	// by its name or account number within a given key scope. It returns an
	// AccountInfo struct containing the account's properties or an error if
	...
	// identified by its old name or its account number. It returns an error
...
type AddressStore interface {
	...
	// fails.
	NewAddress(ctx context.Context, params NewAddressParams) (btcutil.Address, error)
	...
	// fails.
	ImportAddress(ctx context.Context, params ImportAddressParams) (AddressInfo, error)

	// GetAddress retrieves information about a specific address. It returns
	// an AddressInfo struct containing the address's properties or an error
	...
	// imports.
	GetPrivateKey(ctx context.Context, addr btcutil.Address) (*btcec.PrivateKey, error)
}
...
type TxStore interface {
	...
	// any transactions within those blocks back to the unconfirmed pool.
	// This operation is performed as a single, atomic database transaction
	// to ensure data integrity. Breaking it into smaller, separate interface
	// methods would risk leaving the database in an inconsistent state if an
	// error occurred mid-process. The current approach guarantees that the
	// rollback is either fully completed or not at all.
	//
	// TODO(yy): explore performance improvement for this method.
	RollbackToBlock(ctx context.Context, height int32) error
}
...
type UTXOStore interface {
	...
	// LeaseOutput locks a specific UTXO for a given duration, preventing it
	// from being used in coin selection. This is useful for reserving UTXOs
	// for a specific purpose, such as a pending transaction. The method
	...
	// Balance returns the total spendable balance of the wallet, calculated
	// from the UTXO set. The minConfirms parameter specifies the minimum
	// number of confirmations a UTXO must have to be included in the
	// balance calculation.
