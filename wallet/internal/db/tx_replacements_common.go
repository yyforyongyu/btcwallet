...
var (
	...
	errReplacementVictimInvalid = errors.New(
		"replacement victim must be live, unconfirmed, and non-coinbase",
	)
...
// rollbackOrphanRoot records one coinbase transaction row that will become an
// orphan root during block rollback.
type rollbackOrphanRoot struct {
	// WalletID scopes the orphan root to one wallet.
	WalletID uint32

	// TxID identifies the root transaction row to orphan.
	TxID int64
}
...
// applyRollbackOrphanInvalidation groups rollback-created orphan roots by
// wallet and recursively fails every descendant branch in the same SQL
// transaction.
func applyRollbackOrphanInvalidation(ctx context.Context,
	roots []rollbackOrphanRoot,
	buildHooks func(uint32) txChainHooks) error {

	if len(roots) == 0 {
		return nil
	}

	rootsByWallet := make(map[uint32][]int64, len(roots))
	for _, root := range roots {
		rootsByWallet[root.WalletID] = append(
			rootsByWallet[root.WalletID], root.TxID,
		)
	}

	for walletID, rootIDs := range rootsByWallet {
		err := applyTxChainInvalidation(
			ctx, rootIDs, TxStatusOrphaned, buildHooks(walletID),
		)
		if err != nil {
			return fmt.Errorf("wallet %d orphan invalidation: %w",
				walletID, err)
		}
	}
...
// roots.
func buildDirectConflictMeta(id int64, txHashBytes []byte, statusString string,
	hasBlock bool, isCoinbase bool) (txChainMeta, bool, error) {

	status, err := parseTxStatus(statusString)
	if err != nil {
...
// validateReplacementVictim checks that a replacement victim is still an
