...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.clampWalletSyncStateHeightsForRollbackStmt, err = db.PrepareContext(ctx, ClampWalletSyncStateHeightsForRollback); err != nil {
		return nil, fmt.Errorf("error preparing query ClampWalletSyncStateHeightsForRollback: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.clampWalletSyncStateHeightsForRollbackStmt != nil {
		if cerr := q.clampWalletSyncStateHeightsForRollbackStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing clampWalletSyncStateHeightsForRollbackStmt: %w", cerr)
		}
	}
...
type Queries struct {
	...
	clampWalletSyncStateHeightsForRollbackStmt  *sql.Stmt
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		clampWalletSyncStateHeightsForRollbackStmt:  q.clampWalletSyncStateHeightsForRollbackStmt,
