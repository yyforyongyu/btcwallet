...
func TestCreateDerivedAccountConcurrent(t *testing.T) {
	...
	for i := range workers {
		...
		go func(i int) {
			defer wg.Done()
			info, err := store.CreateDerivedAccount(ctx,
...
func TestRenameAccountErrors(t *testing.T) {
	...
	invalidName := "nonexistent"
	invalidNum := uint32(99999)
...
func TestCreateDerivedAccountMaxAccountNumber(t *testing.T) {
	...
	info, err := store.CreateDerivedAccount(t.Context(), db.CreateDerivedAccountParams{
		WalletID: walletID,
		Scope:    db.KeyScopeBIP0084,
		Name:     "account-max",
	})
	...
	// overflows uint32.
	_, err = store.CreateDerivedAccount(t.Context(), db.CreateDerivedAccountParams{
		WalletID: walletID,
		Scope:    db.KeyScopeBIP0084,
