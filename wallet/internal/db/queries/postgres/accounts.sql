...
WHERE
    ...
    AND (
        ...
        OR (
            NOT sqlc.arg('after_imported')::BOOLEAN
            AND (
                (
                    a.account_number IS NOT NULL
                    AND (
                        a.account_number > sqlc.arg('after_account_number')::BIGINT
                        OR (
                            a.account_number = sqlc.arg('after_account_number')::BIGINT
                            AND a.id > sqlc.arg('after_row_id')::BIGINT
                        )
                    )
                )
        ...
        OR (
            sqlc.arg('after_imported')::BOOLEAN
