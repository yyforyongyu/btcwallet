...
--   one stored edge.
-- - Leaves created_at to the table default, which normalizes the timestamp to
--   UTC before storing it in the TIMESTAMP column.
-- Performance:
...
-- - Orders by created_at, which is stored in UTC so traversal order is stable
--   across sessions.
-- Performance:
