...
--
-- NOTE: expires_at is stored as TIMESTAMPTZ. Callers should pass a UTC-normalized
-- timestamp if they want a canonical wire encoding, but comparisons use plain
-- timestamptz values so session timezone settings do not change lease semantics.
-- Performance:
...
-- - Filters out expired rows using the database's timestamptz clock.
...
--   untouched.
-- - Uses the database's timestamptz clock so session timezone settings do not
