...
--
-- SQLite foreign-key actions evaluate row checks before AFTER UPDATE triggers
-- on the child table run. The BEFORE DELETE trigger on blocks therefore
...
BEGIN
    ...
    SET
        ...
        status = CASE
            WHEN is_coinbase THEN 'orphaned'
            ELSE status
        END,
        block_height = NULL
