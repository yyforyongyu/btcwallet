# Guide to PSBT Workflows using the PsbtManager
...
2.  **Fund the PSBT:** Alice's wallet performs coin selection to add inputs and a
    ...
    intent := &wallet.FundIntent{
        MinConfs: 1,
        FeeRate:  wallet.SatPerKVByte(10000), // 10 sat/vB
        FundSource: &wallet.ScopedAccount{
            AccountName: "default",
            KeyScope:    waddrmgr.KeyScopeBIP0086,
        },
    }
    _, err = aliceWallet.FundPsbt(ctx, packet, intent)
    ```
    The `packet` now contains the necessary inputs (fully decorated) and a
    change output.

3.  **Sign the PSBT:** Alice's wallet signs the inputs it just added.

    ```go
    err = aliceWallet.SignPsbt(ctx, packet)
    ```
...
### The Naive (and Insecure) `JoinPsbts` Model - **DO NOT USE**
...
3.  **Coordinator Joins:** A coordinator merges these.
    `combinedPacket, _ := wallet.JoinPsbts(packet_alice, packet_bob)`
...
When the coordinator calls `JoinPsbts`, this malicious output is merged into the
final transaction. If Alice's application logic does not manually parse and
...
err = bobWallet.SignPsbt(ctx, barePacket)
