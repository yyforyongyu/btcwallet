...
func (w *Wallet) prepareTxAuthSources(ctx context.Context, intent *TxIntent) (
	...
	err = walletdb.Update(w.cfg.DB, func(dbtx walletdb.ReadWriteTx) error {
		...
		account, err := manager.LookupAccount(
			addrmgrNs, changeAccount.AccountName,
		)
		if err != nil {
		...
		_, changeSource, err = w.addrMgrWithChangeSource(
			dbtx, &changeAccount.KeyScope, account,
		)
...
func constantInputSource(eligible []db.UtxoInfo) txauthor.InputSource {
...
func constantCreditInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	for i := range eligible {
		...
		if eligible[i].Height >= 0 {
			height = nonNegativeHeightToUint32(eligible[i].Height)
		}
	...
	return constantInputSource(utxos)
}
...
func (w *Wallet) filterEligibleOutputs(ctx context.Context,
	...
	for i := range unspent {
		...
		spendable, utxoAccountName, addressType, err :=
			w.store.GetAddressDetails(ctx, db.GetAddressDetailsQuery{
				WalletID:     w.id,
		...
		if !spendable || utxoAccountName != accountName {
			continue
		}

		addrType := waddrmgr.AddressType(addressType)
