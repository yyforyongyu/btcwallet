...
var (
	...
	// errUnknownChangeScope is returned when the change source references an
	// unknown key scope.
	errUnknownChangeScope = errors.New("unknown change scope")
...
func (w *Wallet) prepareTxAuthSources(ctx context.Context, intent *TxIntent) (
	...
	changeSource, err := w.storeChangeSource(ctx, changeAccount)
	if err != nil {
		return nil, nil, err
	}
...
// storeChangeSource creates a store-backed change source for transaction
// authoring.
func (w *Wallet) storeChangeSource(ctx context.Context,
	changeAccount *ScopedAccount) (*txauthor.ChangeSource, error) {

	accountName := changeAccount.AccountName

	accountInfo, err := w.store.GetAccount(ctx, db.GetAccountQuery{
		WalletID: w.id,
	...
	schema, ok := db.ScopeAddrMap[db.KeyScope(changeAccount.KeyScope)]
	if !ok {
		return nil, fmt.Errorf("%w: %v", errUnknownChangeScope,
			changeAccount.KeyScope)
	}

	storeAddrType := schema.InternalAddrType
...
func (w *Wallet) createManualInputSource(ctx context.Context,
	...
	locked, err := w.leasedOutputSet(ctx)
	if err != nil {
		return nil, err
	}
	...
	for _, outpoint := range inputs.UTXOs {
		...
		credit, err := w.store.GetUtxo(ctx, db.GetUtxoQuery{
			WalletID: w.id,
			OutPoint: outpoint,
		})
		if err != nil {
...
func (w *Wallet) filterEligibleOutputs(ctx context.Context,
	keyScope *waddrmgr.KeyScope, accountName string, account uint32,
	minconf uint32, bs *waddrmgr.BlockStamp) ([]db.UtxoInfo, error) {

	unspent, err := w.store.ListUTXOs(ctx, db.ListUtxosQuery{
		WalletID: w.id,
		Account:  &account,
	})
	if err != nil {
	...
	for i := range unspent {
		...
		if _, ok := locked[output.OutPoint]; ok {
			continue
		}
		...
		addressInfo, err := w.store.GetAddress(ctx, db.GetAddressQuery{
			WalletID:     w.id,
		...
		if addressInfo.IsWatchOnly || addressInfo.AccountName != accountName {
			continue
		}

		addrType := waddrmgr.AddressType(addressInfo.AddrType)

		utxoScope, err := addrType.KeyScope()
		if err != nil {
			continue
		}

		if keyScope != nil && utxoScope != *keyScope {
			continue
		}
...
// leasedOutputSet returns the wallet's currently leased outpoints.
func (w *Wallet) leasedOutputSet(ctx context.Context) (
	...
	locked := make(map[wire.OutPoint]struct{}, len(leases))
	for i := range leases {
		locked[leases[i].OutPoint] = struct{}{}
	}
