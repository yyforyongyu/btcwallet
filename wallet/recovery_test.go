...
func TestGetBranchState(t *testing.T) {
	...
	rs := NewRecoveryState(10, &chaincfg.TestNet3Params, addrMgr)
...
func TestProcessBlock(t *testing.T) {
	...
	props := &waddrmgr.AccountProperties{
		KeyScope:         scope,
		AccountNumber:    0,
		ExternalKeyCount: 0, // Start fresh for easier expansion testing
		InternalKeyCount: 0,
	}
	...
	//    - Finding address at index 5 (External) should make next unfound
	//      6.
	//      Horizon expands to 6 + window (10) = 16. New addresses derived
	//      from 10 to 15.
	for i := uint32(10); i < 16; i++ {
		setupDerive(0, i)
	}

	//    - Finding address at index 12 (External) should make next unfound
	//      13.
	//      Horizon expands to 13 + window (10) = 23. New addresses derived
	//      from 16 to 22.
	...
	// Tx2: Pays to Addr 12 (External Branch) - an address initially outside
	//      the lookahead, but becomes visible after the first expansion.
...
func TestBuildCFilterData(t *testing.T) {
	...
	addr1, _ := btcutil.DecodeAddress(
		"mrCDrCybB6J1vRfbwM5hemdJz73FwDBC8r", &chaincfg.TestNet3Params,
	)
...
func TestInitAccountState(t *testing.T) {
	...
	// RecoveryWindow 2.
	rs := NewRecoveryState(2, &chaincfg.TestNet3Params, addrMgr)
	rs.addrFilters = make(map[string]AddrEntry)
...
// addresses are reported as found. This test specifically checks that
// new addresses are derived (via the mocked AccountStore) to maintain
// the recovery window, and that these newly derived addresses are
// subsequently added to the RecoveryState's transient addrFilters.
...
// that the corresponding BranchRecoveryState's internal `nextUnfound` value
// is updated appropriately based on these findings, triggering potential
// future horizon expansions.
...
// RecoveryState's configured address filters and watched outpoints.
// This test ensures that the method correctly identifies credits (payments
// to our addresses) and debits (spends from our outpoints), updates the
// transient outpoints map (removing spent inputs, adding new relevant outputs),
// and populates the foundScopes and relevantOutputs maps for subsequent
// processing.
func TestFilterTx(t *testing.T) {
	t.Parallel()

	rs := NewRecoveryState(10, &chaincfg.TestNet3Params, nil)
	rs.addrFilters = make(map[string]AddrEntry)
	rs.outpoints = make(map[wire.OutPoint][]byte)

	// 1. Setup Watched Address.
	// Use real address for Script parsing interaction with txscript.
	addr, _ := btcutil.DecodeAddress("mrCDrCybB6J1vRfbwM5hemdJz73FwDBC8r",
...
// TestRecoveryStateStringAndEmpty verifies the String and Empty methods
// of RecoveryState. It ensures that the String method produces a non-empty
...
func TestInitializeWithState(t *testing.T) {
	...
	err := rs.Initialize(nil, []btcutil.Address{addr},
...
func TestProcessBlockError(t *testing.T) {
	...
	realAddr, _ := btcutil.NewAddressPubKeyHash(make([]byte, 20),
		&chaincfg.TestNet3Params)
