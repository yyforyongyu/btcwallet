...
const (
	// syncStateSwitchThreshold is the number of blocks behind the chain tip
	// at which the wallet switches to the "Syncing" state. Gaps smaller
	// than this are handled silently (blocking DB lock) to avoid disrupting
	// UX with "Wallet Busy" errors for minor lags.
	//
	...
	// lock. Processing 6 blocks typically takes less than 1 second, which
	// is an acceptable duration for other operations (like CreateTx) to
	// block (wait) on the database lock. Gaps larger than this would result
	// in noticeable UI hangs, so we switch to the explicit "Syncing" state
	// which allows the wallet to return an immediate error instead of
	// blocking.
	//
	// 2. Data Integrity vs. UX: While in a silent sync, the wallet might
	// allow the user to initiate actions based on slightly outdated data
	// (e.g., spending an output that was actually spent in one of the
	// missing blocks). For a 6-block gap, the risk is minimal, and such
	// transactions would be rejected by the network mempool or miners.
	// However, for large gaps, the risk of false "Insufficient Funds"
	// errors or extremely inaccurate fee estimates increases, making the
	// explicit "Syncing" state a necessary safeguard.
...
func (s *syncer) initResultsForCFilterScan(ctx context.Context,
	...
	for i := range results {
		...
		if filter == nil || filter.N() == 0 {
			log.Errorf("Filter missing or empty for block %v "+
				...
				results[i].meta.Hash, filter == nil, filter.N())
...
// fetchAndFilterBlocks retrieves and processes a batch of blocks from the chain
// backend. It handles CFilter matching, block fetching, local filtering, and
// dynamic address discovery (expanding horizons in memory/read-only DB).
func (s *syncer) fetchAndFilterBlocks(ctx context.Context,
	scanState *RecoveryState, startHeight, chainTip int32) ([]scanResult,
	error) {
...
// maxFilterItems is the heuristic threshold for the number of items (addresses
// + outpoints) in the watchlist at which the cost of client-side GCS filter
// matching exceeds the cost of downloading and parsing full blocks.
//
// Calculation:
...
func (s *syncer) dispatchScanStrategy(ctx context.Context,
	...
	case SyncMethodAuto:
		...
		if scanState.WatchListSize() > maxFilterItems {
			log.Infof("Auto sync: Watchlist size %d > %d, "+
				"switching to full blocks for performance",
				scanState.WatchListSize(), maxFilterItems)
...
func (s *syncer) handleChainUpdate(ctx context.Context, n any) error {
	...
	case *chain.RescanFinished:
		// Consume and log the legacy RescanFinished notification. We
		// no longer perform state updates here as the new controller-
		// driven sync loop manages wallet synchronization.
...
func (s *syncer) processChainUpdate(ctx context.Context, update any) error {
	...
	case chain.BlockDisconnected:
		// A block was disconnected. We use checkRollback to safely
		// verify our chain state against the backend and rewind if
		// necessary. This handles both single block disconnects and
		// deeper reorgs robustly.
...
// loadTargetedScanState initializes a recovery state for a targeted
// rescan of specific accounts.
