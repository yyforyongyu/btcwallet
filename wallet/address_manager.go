...
func deriveStoreAddress(params db.AddressDerivationParams,
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("parse account pubkey: %w", err)
	}
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("derive address index: %w", err)
	}
	defer addrKey.Zero()

	pubKey, err := addrKey.ECPubKey()
	if err != nil {
		return nil, nil, nil, fmt.Errorf("derive address pubkey: %w", err)
	}
	...
	addr, err := walletAddrType.AddrFromPubKeyBytes(pubKeyBytes, chainParams)
	if err != nil {
