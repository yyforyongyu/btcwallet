...
func (b baseUnit) ToVB() VByte {
	...
	//
	// We use integer arithmetic (wu + 3) / 4 to achieve the ceiling
	// division without converting to float64.
	vbytes := (b.wu + blockchain.WitnessScaleFactor - 1) /
		blockchain.WitnessScaleFactor

	return NewVByte(vbytes)
}

// ToKVB converts the unit to a KVByte.
func (b baseUnit) ToKVB() KVByte {
	// KVByte is a multiple of VByte, so we align to VByte first.
	return KVByte{b.ToVB().baseUnit}
}
